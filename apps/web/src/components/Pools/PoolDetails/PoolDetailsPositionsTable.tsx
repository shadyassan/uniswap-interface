...
function PositionRow({ positionInfo }: { positionInfo: PositionInfo }) {
  ...
  const onClick = useCallback(async () => {
    if (walletChainId !== positionInfo.chainId)
      await switchChain(connector, positionInfo.chainId);
    navigate('/pool/' + positionInfo.details.tokenId);
  }, [
    connector,
    navigate,
    positionInfo.chainId,
    positionInfo.details.tokenId,
    switchChain,
    walletChainId,
  ]);
    ...
    <PositionWrapper onClick={onClick}>
      <Row gap="8px">
        ...
        <StatusWrapper status={status}>
          ...
          {status === PositionStatus.OUT_OF_RANGE && (
            <AlertTriangle size={12} color={theme.deprecated_accentWarning} />
          )}
...
export function PoolDetailsPositionsTable({
  positions,
}: {
