...
const HEADER_DESCRIPTIONS: Record<PoolSortFields, ReactNode | undefined> = {
  ...
  [PoolSortFields.Turnover]: (
    <Trans>
      Turnover refers to the amount of trading volume relative to total value
      locked (TVL) within a pool. Turnover = 24H Fees / TVL
    </Trans>
  ),
};
    ...
    () =>
      pools?.map((pool, index) => {
        ...
        return {
          ...
          link: `/explore/pools/${chainIdToBackendName(
            chainId
          ).toLowerCase()}/${pool.hash}`,
          analytics: {
        ...
        ? columnHelper.accessor((row) => row.turnover, {
            ...
            cell: (turnover) => (
              <Cell minWidth={100} loading={showLoadingSkeleton} grow>
                <ThemedText.BodyPrimary>
                  {formatPercent(turnover.getValue?.())}
                </ThemedText.BodyPrimary>
              </Cell>
            ),
          })
