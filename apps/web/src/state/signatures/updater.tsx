...
export default function Updater() {
    ...
    () =>
      Object.values(signatures).filter(
        (signature) =>
          ...
          ].includes(signature.type as SignatureType) &&
  ...
  const onOrderUpdate = useCallback(
    (order: UniswapXOrderDetails, update: UniswapXBackendOrder) => {
      ...
      const popupDismissalTime = isL2ChainId(order.chainId)
        ? L2_TXN_DISMISS_MS
      ...
      if (update.orderStatus === UniswapXOrderStatus.FILLED && update.txHash) {
        ...
        if (updatedOrder.swapInfo.tradeType === TradeType.EXACT_INPUT) {
          updatedOrder.swapInfo = {
            ...
            settledOutputCurrencyAmountRaw:
              update.settledAmounts?.[0]?.amountOut,
          };
        }
        ...
        provider?.getTransactionReceipt(update.txHash).then((receipt) => {
          ...
          addPopup(
            { type: PopupType.Transaction, hash: update.txHash },
            update.txHash,
            popupDismissalTime
          );
        });
        ...
        dispatch(updateSignature(updatedOrder));
        addPopup(
          { type: PopupType.Order, orderHash: order.orderHash },
  ...
  return (
    <OrderUpdater pendingOrders={pendingOrders} onOrderUpdate={onOrderUpdate} />
  );
}
